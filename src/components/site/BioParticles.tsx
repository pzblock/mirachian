...
const SPECKS = Array.from({ length: COUNT }, (_, i) => {
  ...
  const size = sizeRoll > 92 ? 16 : sizeRoll > 72 ? 11 : 7;
  const top = (i * 13) % 96;
...
export function BioParticles() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      el.style.opacity = String(0.22 + p * 0.7);
    };
    onScroll();
  ...
  return (
    <div ref={ref} className="bio-field" style={{ opacity: 0.22 }} aria-hidden>
      {SPECKS.map((s) => (
        <span
          key={s.i}
